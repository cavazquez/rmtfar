...
//!
//! Mumble's built-in Link plugin reads a well-known shared memory segment and
//! applies 3D positional audio automatically. We write the local player's
//! position and orientation here so Mumble handles proximity falloff natively.
//!
...
/// Mumble Link shared memory layout.
#[repr(C)]
struct LinkedMem {
    ui_version: u32,
    ui_tick: u32,
...
impl MumbleLink {
    pub fn new() -> Self {
        match Inner::open() {
            Ok(inner) => {
                debug!("Mumble Link shared memory opened");
                Self { inner: Some(inner) }
            }
            Err(e) => {
                warn!("Mumble Link not available (is Mumble running?): {e}");
                Self { inner: None }
            }
        }
    }
